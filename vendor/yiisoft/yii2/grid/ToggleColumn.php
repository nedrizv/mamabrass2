...
class ToggleColumn extends DataColumn
{
    ...
    {
	/* @Edited By AmitG  */
	if(get_class($model) == 'admin\models\BlogCategories')
        	$url = [$this->action, 'id' => $model->blog_category_id];
		
	if(get_class($model) == 'admin\models\Blog')
        	$url = [$this->action, 'id' => $model->blog_id];
		
	if(get_class($model) == 'admin\models\Categories')
        	$url = [$this->action, 'id' => $model->category_id];
		
	if(get_class($model) == 'admin\models\SubCategories')
        	$url = [$this->action, 'id' => $model->subcat_id];	
		
	if(get_class($model) == 'admin\models\Country')
		$url = [$this->action, 'id' => $model->country_code];
